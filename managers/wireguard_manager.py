...
def _split_clients_snapshot(raw: str) -> dict:
    ...
    current = None
    key = {'__AMNZ_TABLE__': 'table', '__AMNZ_SHOW__': 'show', '__AMNZ_CONF__': 'conf'}
...
"""
    ...
    def _parse_peers_from_config(self, config=None):
    ...
    def _parse_wg_show_output(self, raw):
        result = {}
        current_peer = None
    ...
    def _wg_show(self):
        ...
        if code != 0 or not out.strip():
            return {}
        return self._parse_wg_show_output(out)

    def get_clients(self):
        """Get list of all clients with live traffic data."""
        try:
            script = (
                f'printf "%s\\n" __AMNZ_TABLE__; cat {self.CLIENTS_TABLE_PATH} 2>/dev/null || true; '
                f'printf "\\n%s\\n" __AMNZ_SHOW__; wg show all 2>/dev/null || true; '
                f'printf "\\n%s\\n" __AMNZ_CONF__; cat {self.CONFIG_PATH} 2>/dev/null || true'
            )
            out, _, _ = self.ssh.run_sudo_command(
                f"docker exec -i {self.CONTAINER_NAME} sh -c '{script}'",
                timeout=20,
            )
            chunks = _split_clients_snapshot(out)
            clients_table = []
            if chunks['table'].strip():
                try:
                    parsed = json.loads(chunks['table'])
                    if isinstance(parsed, list):
                        clients_table = parsed
                    elif isinstance(parsed, dict):
                        clients_table = [
                            {
                                'clientId': cid,
                                'userData': {'clientName': (info or {}).get('clientName') or (info or {}).get('name') or 'Unknown'},
                            }
                    ...
                    clients_table = []
            wg_show_data = self._parse_wg_show_output(chunks['show'])
            conf_text = chunks['conf']
        ...
        try:
            conf_peers = self._parse_peers_from_config(conf_text)
